...
export async function generateQuestionsFromDocument(content, topic, options = {}) {
  try {
    ...
    const prompt = `
      ...
      1. **שפת המבחן:** חובה עליך לזהות את השפה שבה כתוב מסמך המקור, ולכתוב את כל השאלות והתשובות **בדיוק באותה השפה** (לדוגמה: אם המסמך נכתב באנגלית, נסח את כל המבחן באנגלית).
      2. חובה לבסס את כל השאלות והתשובות אך ורק על המסמך המצורף.
      3. אסור לך להמציא שאלות או נתונים מהידע הכללי שלך בשום אופן! 
    ...
    console.log(`✅ AI Engine: חולצו בהצלחה ${parsedData.length} שאלות בשפת המקור.`);
    return parsedData;
