...
export function TrainingScreen({ 
    ...
    return (
        <div className="screen-layout" style={{ direction: "rtl", fontFamily: "sans-serif" }}>
            ...
            {/* Chat Area - האזור האמצעי שנגלל */}
            <div className="screen-content" style={{ padding: "40px 20px", display: "flex", flexDirection: "column", gap: "20px", alignItems: "center" }}>
                {msgs.map((m, i) => {
                    ...
                    let borderColor = "transparent"; let textColor = "#f8fafc";
                    ...
                    return (
                        <div key={i} style={{ width: "100%", maxWidth: "800px", display: "flex", flexDirection: "column", alignItems: isAI ? "flex-start" : "flex-end" }}>
                            ...
                            <div style={{ background: "#0f172a", color: textColor, padding: "20px 30px", borderRadius: "12px", border: isSystemReveal || isSystemHint ? `1px solid ${borderColor}` : "1px solid #1e293b", fontSize: "16px", lineHeight: "1.6", textAlign: "right", maxWidth: "90%", boxShadow: "0 4px 6px -1px rgba(0, 0, 0, 0.1)", whiteSpace: "pre-wrap" }}>
                                {m.text}
                            </div>
                        </div>
                    );
                ...
                {inlineTimer !== null && (
                    <div style={{ width: "100%", maxWidth: "800px", display: "flex", justifyContent: "flex-start", marginTop: "10px" }}>
                        <div style={{ background: "rgba(249, 115, 22, 0.08)", color: "#f97316", padding: "8px 16px", borderRadius: "20px", fontSize: "13px", border: "1px solid rgba(249, 115, 22, 0.2)", display: "flex", alignItems: "center", gap: "8px", fontWeight: "500" }}>
            ...
            <div className="screen-footer" style={{ padding: "20px", background: "#0b1120", borderTop: "1px solid #1e293b", display: "flex", justifyContent: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "15px", width: "100%", maxWidth: "800px" }}>
                    <button onClick={sendAnswer} disabled={loading || !input.trim() || inlineTimer !== null} style={{ background: "transparent", color: (loading || !input.trim() || inlineTimer !== null) ? "#334155" : "#4ade80", border: "none", cursor: (loading || !input.trim() || inlineTimer !== null) ? "not-allowed" : "pointer", padding: "10px" }}>
                        <Send size={24} style={{ transform: "scaleX(-1)" }} />
                    </button>
                    <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !loading && input.trim() && inlineTimer === null && sendAnswer()} placeholder={inlineTimer !== null ? "המתן לשאלה הבאה..." : "הקלד את תשובתך ..."} disabled={inlineTimer !== null} style={{ flex: 1, padding: "16px", borderRadius: "10px", border: "1px solid #1e293b", background: "#0f172a", color: "#fff", fontSize: "16px", outline: "none", opacity: inlineTimer !== null ? 0.5 : 1 }} />
                    <div style={{ display: "flex", gap: "10px" }}>
                        <button disabled={!isRevealActive || inlineTimer !== null} onClick={handleRevealClick} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "10px 15px", borderRadius: "8px", background: "transparent", cursor: (!isRevealActive || inlineTimer !== null) ? "not-allowed" : "pointer", color: (!isRevealActive || inlineTimer !== null) ? "#334155" : "#f97316", border: `1px solid ${(!isRevealActive || inlineTimer !== null) ? "#1e293b" : "#f97316"}`, transition: "all 0.3s", whiteSpace: "nowrap" }} title="פעיל אחרי 3 טעויות">
                            <Eye size={18} /> גלה תשובה
                        </button>
                        <button disabled={!isHelpActive || inlineTimer !== null} onClick={handleHelpClick} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "10px 15px", borderRadius: "8px", background: "transparent", cursor: (!isHelpActive || inlineTimer !== null) ? "not-allowed" : "pointer", color: (!isHelpActive || inlineTimer !== null) ? "#334155" : "#38bdf8", border: `1px solid ${(!isHelpActive || inlineTimer !== null) ? "#1e293b" : "#38bdf8"}`, transition: "all 0.3s", whiteSpace: "nowrap" }} title="פעיל אחרי טעות אחת">
                            <HelpCircle size={18} /> עזרה
                        </button>
                    </div>
                </div>
            </div>

            {/* פופאפים של סיום ואישור (נשארים אותו דבר) */}
            {showFinishModal && (
                <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(2,6,23,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
                    <div style={{ background: "#0f172a", padding: "40px", borderRadius: "20px", textAlign: "center", border: "1px solid #334155", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.5)", maxWidth: "450px" }}>
                        ...
                        <p style={{ color: "#94a3b8", marginBottom: "30px", lineHeight: "1.5" }}>האם אתה בטוח שברצונך לסיים את האימון כעת?</p>
                        <div style={{ display: "flex", gap: "15px", justifyContent: "center" }}>
                            <button onClick={() => { setShowFinishModal(false); setScreen("debrief"); }} style={{ padding: "10px 24px", fontSize: "16px", borderRadius: "8px", background: "#3b82f6", color: "#fff", border: "none", cursor: "pointer", fontWeight: "bold" }}>כן, עבור לתחקיר</button>
                            <button onClick={() => setShowFinishModal(false)} style={{ padding: "10px 24px", fontSize: "16px", borderRadius: "8px", background: "transparent", color: "#94a3b8", border: "1px solid #334155", cursor: "pointer" }}>ביטול</button>
                        </div>
                    </div>
                </div>
            )}
